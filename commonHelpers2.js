import{c as I,o as L,a as k,b as P}from"./assets/openbook-2238cf13.js";import{P as u,a as G}from"./assets/vendor-9b4a9007.js";function Q(t){t.classList.add("loader")}function B(t){t.classList.remove("loader")}const v="data:image/png;base64,",C="data:image/png;base64,";async function R(t){return(await G.get(`https://books-backend.p.goit.global/books/${t}`)).data}const g=document.querySelector(".js-books-container"),h=document.querySelector(".empty-shopping-list-main"),d=document.querySelector("#shopping-list-loader"),l=document.getElementById("pagination2");let i,A=b();I();y();f();function y(){g.innerHTML="",h.style.display="none",Q(d)}window.addEventListener("resize",()=>{i.length!==0&&(A=b(),new u(l,{totalItems:i.length,itemsPerPage:A,visiblePages:3,centerAlign:!0}),p(1))});function E(t,s){return new Promise(function(e,n){function a(o){t.removeEventListener(s,a),e(o)}t.addEventListener(s,o=>{o.target.classList.contains("tui-page-btn")&&!o.target.classList.contains("tui-is-selected")&&!o.target.classList.contains("tui-is-disabled")&&a()})})}function b(){return window.innerWidth<768?4:3}function f(){U().then(t=>{i=t,i.length===0?(B(d),h.style.display="block",g.innerHTML="",l.innerHTML=""):(B(d),new u(l,{totalItems:i.length,itemsPerPage:A,visiblePages:3,centerAlign:!0}),p(1))}).catch(()=>{L()})}function p(t){const n=[...i].splice(A*(t-1),A).map(a=>Y(a)).join("");g.innerHTML=n,h.style.display="none",E(l,"click").then(function(a){p(parseInt(document.getElementsByTagName("strong")[0].innerHTML))})}function U(){return new Promise((t,s)=>{const e=localStorage.getItem("books");if(e){const n=JSON.parse(e),a=[];n.forEach(o=>{const c=R(o);a.push(c)}),Promise.all(a).then(o=>{t(o)}).catch(o=>{s(o)})}else t([])})}function Y(t){const{book_image:s,title:e,list_name:n,description:a,author:o,buy_links:c,_id:m}=t;return`<div class="book-list">
<img class="book-card-img" src="${s}" alt="${e}" />
<div class="shop-card-info">
<div class="shop-card-hero">
<div class="shop-list-title-name">
<h3 class="shop-list-title">${e}</h3>
<p class="shop-list-list-name">${n}</p>
</div>
<button class="remove-shop-list-book" data-bookid="${m}">
<img 
  class="icon-basket-shop-list"
  data-bookid="${m}"
  src="${C}"
  alt="Delete book"
  />
</button>
</div>
<p class="shop-list-description">${a}<p>
<div class="shop-list-downpart">
<p class="shop-list-author"> Author: ${o}</p>
<div class="shop-list-links">
<a class="amazon-link" target="_blank" href="${c.find(r=>r.name==="Amazon").url}">
<img 
class="amazon-img  html.dark-mode & {}"
srcset="
${k} 1x,
${v} 2x
"
src="${k}"
alt="Amazon Shop"
/>
</a>
<a class="openbooks-link" target="_blank" href="${c.find(r=>r.name==="Apple Books").url}">
     <img 
  class="open-book-icon"
  src="${P}"
  alt="Open book"
  />
  </a>
</div>
</div>
</div>
</div>`}function j(t){if(t.target.classList.contains("remove-shop-list-book")||t.target.classList.contains("icon-basket-shop-list")||t.target.dataset.name==="remove-shop-list-book"){const s=t.target.getAttribute("data-bookid");H(s)}}function H(t){let s=localStorage.getItem("books");if(s){s=JSON.parse(s);const e=s.indexOf(t);e!==-1&&s.splice(e,1),localStorage.setItem("books",JSON.stringify(s)),f()}}document.addEventListener("click",j);
//# sourceMappingURL=commonHelpers2.js.map

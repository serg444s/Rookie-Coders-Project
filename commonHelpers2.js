import"./assets/styles-c6c4fe92.js";import{a as b}from"./assets/vendor-375581c1.js";function m(o){o.classList.add("loader")}function h(o){o.classList.remove("loader")}const c="data:image/png;base64,",k="data:image/png;base64,";async function B(o){return(await b.get(`https://books-backend.p.goit.global/books/${o}`)).data}const r=document.querySelector(".js-books-container"),d=document.querySelector(".empty-shopping-list-main"),l=document.querySelector("#shopping-list-loader");E();p();function E(){r.innerHTML="",d.style.display="none",m(l)}function p(){f().then(o=>{const s=o;if(s.length===0)h(l),d.style.display="block",r.innerHTML="";else{const a=s.map(t=>u(t)).join("");h(l),r.innerHTML=a,d.style.display="none"}}).catch(o=>{console.error(o)})}function f(){return new Promise((o,s)=>{const a=localStorage.getItem("books");if(a){const t=JSON.parse(a),e=[];t.forEach(A=>{const i=B(A);e.push(i)}),Promise.all(e).then(A=>{o(A)}).catch(A=>{s(A)})}else o([])})}function u(o){const{book_image:s,title:a,list_name:t,description:e,author:A,buy_links:i,_id:g}=o;return`<div class="book-list">
<img class="book-card-img" src="${s}" alt="${a}" />
<div class="shop-card-info">
<div class="shop-card-hero">
<div class="shop-list-title-name">
<h3 class="shop-list-title">${a}</h3>
<p class="shop-list-list-name">${t}</p>
</div>
<button class="remove-shop-list-book" data-bookid="${g}">
<svg class="icon-basket-shop-list" width="28" height="26" data-bookid="${g}">
 <use data-name="icon-removebook" href="../img/symbol-defs.svg#icon-removebook"></use>
</svg>
</button>
</div>
<p class="shop-list-description">${e}<p>
<div class="shop-list-downpart">
<p class="shop-list-author"> Author: ${A}</p>
<div class="shop-list-links">
<a class="amazon-link" target="_blank" href="${i.find(n=>n.name==="Amazon").url}">
<img 
class="amazon-img"
srcset="
${c} 1x,
${c} 2x
"
src="${c}"
alt="Amazon Shop"
/>
</a>
<a class="openbooks-link" target="_blank" href="${i.find(n=>n.name==="Apple Books").url}">
     <img 
  class="open-book-icon"
  src="${k}"
  alt="Open book"
  />
  </a>
</div>
</div>
</div>
</div>`}function I(o){if(o.target.classList.contains("remove-shop-list-book")||o.target.classList.contains("icon-basket-shop-list")||o.target.dataset.name==="remove-shop-list-book"){const s=o.target.getAttribute("data-bookid");Q(s)}}function Q(o){let s=localStorage.getItem("books");if(s){s=JSON.parse(s);const a=s.indexOf(o);a!==-1&&s.splice(a,1),localStorage.setItem("books",JSON.stringify(s)),p()}}document.addEventListener("click",I);const G=document.querySelector(".add-book");G.addEventListener("click",o=>{const s=["643282b1e85766588626a080","643282b1e85766588626a0ba","643282b1e85766588626a0dc","643282b2e85766588626a0fc","643282b2e85766588626a118","643282b1e85766588626a085","643282b1e85766588626a0b2"];localStorage.setItem("books",JSON.stringify(s))});
//# sourceMappingURL=commonHelpers2.js.map

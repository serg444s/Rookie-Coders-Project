import{d as I,l as L,a as m,o as P,b as B,c as G}from"./assets/openbook-5f5da516.js";import{P as u,a as Q}from"./assets/vendor-9b4a9007.js";const v="data:image/png;base64,",C="data:image/png;base64,";async function R(t){return(await Q.get(`https://books-backend.p.goit.global/books/${t}`)).data}const g=document.querySelector(".js-books-container"),h=document.querySelector(".empty-shopping-list-main"),d=document.querySelector("#shopping-list-loader"),l=document.getElementById("pagination2");let i,A=b();I();y();f();function y(){g.innerHTML="",h.style.display="none",L(d)}window.addEventListener("resize",()=>{i.length!==0&&(A=b(),new u(l,{totalItems:i.length,itemsPerPage:A,visiblePages:3,centerAlign:!0}),p(1))});function E(t,s){return new Promise(function(a,n){function e(o){t.removeEventListener(s,e),a(o)}t.addEventListener(s,o=>{o.target.classList.contains("tui-page-btn")&&!o.target.classList.contains("tui-is-selected")&&!o.target.classList.contains("tui-is-disabled")&&e()})})}function b(){return window.innerWidth<768?4:3}function f(){U().then(t=>{i=t,i.length===0?(m(d),h.style.display="block",g.innerHTML="",l.innerHTML=""):(m(d),new u(l,{totalItems:i.length,itemsPerPage:A,visiblePages:3,centerAlign:!0}),p(1))}).catch(()=>{P()})}function p(t){const n=[...i].splice(A*(t-1),A).map(e=>Y(e)).join("");g.innerHTML=n,h.style.display="none",E(l,"click").then(function(e){p(parseInt(document.getElementsByTagName("strong")[0].innerHTML))})}function U(){return new Promise((t,s)=>{const a=localStorage.getItem("books");if(a){const n=JSON.parse(a),e=[];n.forEach(o=>{const c=R(o);e.push(c)}),Promise.all(e).then(o=>{t(o)}).catch(o=>{s(o)})}else t([])})}function Y(t){const{book_image:s,title:a,list_name:n,description:e,author:o,buy_links:c,_id:k}=t;return`<div class="book-list">
<img class="book-card-img" src="${s}" alt="${a}" />
<div class="shop-card-info">
<div class="shop-card-hero">
<div class="shop-list-title-name">
<h3 class="shop-list-title">${a}</h3>
<p class="shop-list-list-name">${n}</p>
</div>
<button class="remove-shop-list-book" data-bookid="${k}">
<img 
  class="icon-basket-shop-list"
  data-bookid="${k}"
  src="${C}"
  alt="Delete book"
  />
</button>
</div>
<p class="shop-list-description">${e}<p>
<div class="shop-list-downpart">
<p class="shop-list-author"> Author: ${o}</p>
<div class="shop-list-links">
<a class="amazon-link" target="_blank" href="${c.find(r=>r.name==="Amazon").url}">
<img 
class="amazon-img  html.dark-mode & {}"
srcset="
${B} 1x,
${v} 2x
"
src="${B}"
alt="Amazon Shop"
/>
</a>
<a class="openbooks-link" target="_blank" href="${c.find(r=>r.name==="Apple Books").url}">
     <img 
  class="open-book-icon"
  src="${G}"
  alt="Open book"
  />
  </a>
</div>
</div>
</div>
</div>`}function j(t){if(t.target.classList.contains("remove-shop-list-book")||t.target.classList.contains("icon-basket-shop-list")||t.target.dataset.name==="remove-shop-list-book"){const s=t.target.getAttribute("data-bookid");H(s)}}function H(t){let s=localStorage.getItem("books");if(s){s=JSON.parse(s);const a=s.indexOf(t);a!==-1&&s.splice(a,1),localStorage.setItem("books",JSON.stringify(s)),f()}}document.addEventListener("click",j);
//# sourceMappingURL=commonHelpers2.js.map

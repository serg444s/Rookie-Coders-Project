import{l as p,a as k,o as y,b as $,c as x,d as M}from"./assets/openbook-77ce173d.js";import{a as u,b as T}from"./assets/vendor-9b4a9007.js";async function g(t){return t.map(({author:e,book_image:o,title:a,description:n,_id:c})=>`<li class="books-item-about" >
    <div class="book-wrap">
    ...
    </li>`).join("")}async function f(t){return d()<768>d()?g(t.slice(0,1)):d()<1439>d()?g(await t.slice(0,3)):g(t)}function d(){return window.innerWidth}async function C(t){return(await Promise.all(t.map(async({list_name:o,books:a})=>`
    ...
    `))).join("")}document.addEventListener("click",async t=>{if(t.target.classList.contains("book-button")){const e=t.target.dataset.category;await S(e)}});const m=document.querySelector("#modal-main");async function B(){p(m);const t=document.querySelector(".top-categories-list");try{const e=await u.get("https://books-backend.p.goit.global/books/top-books");t.insertAdjacentHTML("afterbegin",'<h2 class="categories-title">Best Sellers<span> Books</span></h2>');const o=await C(e.data);return t.insertAdjacentHTML("beforeend",o),k(m),e.data}catch(e){console.log(e.message),y()}}async function S(t){p(m);const e=document.querySelector(".top-categories-list");e.innerHTML="";try{const a=(await u.get(`https://books-backend.p.goit.global/books/category?category=${t}`)).data;e.insertAdjacentHTML("afterbegin",await P(t,a)),k(m)}catch(o){console.log(o.message),y()}}async function P(t,e){const o=t.split(" "),n="<span>"+o[o.length-1]+"</span>";return o[o.length-1]=n,`
  <h3 class="categories-title">${o.join(" ")}</h3>
  <ul class='list-books'>${await f(e)}</ul>`}async function h(){const t="https://books-backend.p.goit.global/books/category-list";try{return(await u.get(t)).data}catch(e){console.error("Error fetching books:",e)}}async function D(){try{const t=await h(),e=document.querySelector(".categories-list_books");t.forEach(o=>{const a=document.createElement("li");a.classList.add("item-list_category"),a.dataset.category=o.list_name;const n=document.createElement("a");n.textContent=o.list_name,n.href=`#${o.list_name}`,n.classList.add("link_category-book"),a.appendChild(n),e.appendChild(a),a.addEventListener("click",async()=>{const c=a.dataset.category;await S(c)})})}catch(t){console.error("Error rendering categories:",t)}}document.addEventListener("DOMContentLoaded",D);const L=document.querySelector('[data-action="open-modal"]'),v=document.querySelector('[data-action="close-modal"]'),w=document.querySelector("body");function H(){document.body.classList.add("show-modal"),v.style.display="flex",L.style.display="none",w.style.overflow="hidden"}function N(){document.body.classList.remove("show-modal"),v.style.display="none",L.style.display="flex",w.style.overflow="auto"}let l=JSON.parse(localStorage.getItem("books"))||[];function _(t){l=JSON.parse(localStorage.getItem("books"))||[],l.includes(t)||(l.push(t),localStorage.setItem("books",JSON.stringify(l)))}function I(t){const e=JSON.parse(localStorage.getItem("books")),o=e.findIndex(a=>a===t);o!==-1&&e.splice(o,1),localStorage.setItem("books",JSON.stringify(e))}async function A(t){const e=`https://books-backend.p.goit.global/books/${t}`;return(await u.get(e)).data}const R=document.querySelector(".top-categories-list");document.querySelector(".modal-btn");document.querySelector(".modal-congrat-text");let r;R.addEventListener("click",E);let s;async function E(t){if(t.preventDefault(),t.target.nodeName!=="IMG")return;r=t.target.dataset.id;const e=await A(r);let o;l.includes(r)||(o="ADD TO SHOPPING LIST"),l.includes(r)&&(o="REMOVE FROM THE SHOPPING LIST");const{book_image:a,title:n,author:c,description:O,amazon_product_url:q}=e;s=T.create(`<div class="modal">
